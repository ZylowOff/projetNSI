...
temps_de_course = 0  # Temps écoulé en courant
delai_remplissage = 10000  # Temps pour remplir complètement l'endurance (10 secondes)
delai_course = 5000  # Temps de course maximum (5 secondes)

# Ajouter une variable pour stocker la position intermédiaire du joueur
position_intermediaire = [joueur_pos[0], joueur_pos[1]]

# Ajouter une variable pour stocker la position cible
position_cible = [joueur_pos[0], joueur_pos[1]]
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.KEYDOWN:
            touches = pygame.key.get_pressed()  # Met à jour l'état des touches
            if event.key == pygame.K_ESCAPE:
    ...
    # Déterminer le délai de mouvement en fonction de l'état de la touche Shift
    if touches[pygame.K_LSHIFT] or touches[pygame.K_RSHIFT]:
        if endurance_actuelle > 0:
            delai_mouvement = DELAI_MOUVEMENT_RAPIDE
            endurance_actuelle -= (2 * (1000 / delai_course))  # Diminuer l'endurance deux fois plus vite
            temps_de_course += delai_mouvement  # Augmenter le temps de course
        else:
            delai_mouvement = DELAI_MOUVEMENT_NORMAL  # Revenir au mouvement normal si l'endurance est épuisée
    else:
        delai_mouvement = DELAI_MOUVEMENT_NORMAL
        # Remplir l'endurance deux fois plus vite
        endurance_actuelle += (2 * (1000 / delai_remplissage))  # Remplir l'endurance deux fois plus vite
        endurance_actuelle = min(endurance_max, endurance_actuelle)  # Ne pas dépasser l'endurance max
    ...
    if temps_actuel - dernier_mouvement >= delai_mouvement:
        ...
        nouvelle_pos = [position_cible[0], position_cible[1]]  # Copie de la position cible
        if touches[pygame.K_UP] or touches[pygame.K_z]:
            nouvelle_pos[1] -= 1  # Déplacement vers le haut
        if touches[pygame.K_DOWN] or touches[pygame.K_s]:
            nouvelle_pos[1] += 1  # Déplacement vers le bas
        if touches[pygame.K_LEFT] or touches[pygame.K_q]:
            nouvelle_pos[0] -= 1  # Déplacement vers la gauche
        if touches[pygame.K_RIGHT] or touches[pygame.K_d]:
            nouvelle_pos[0] += 1  # Déplacement vers la droite
        # Convertir les coordonnées en entiers pour la vérification
        nouvelle_pos_int = [int(nouvelle_pos[0]), int(nouvelle_pos[1])]

        # Vérifie si le déplacement est valide
        if deplacement_valide(hopital, nouvelle_pos_int):
            # Mettre à jour la position cible
            position_cible[0] = nouvelle_pos_int[0]
            position_cible[1] = nouvelle_pos_int[1]

        # Mettre à jour le temps du dernier mouvement
        dernier_mouvement = temps_actuel

    # Interpolation pour un mouvement plus fluide
    joueur_pos[0] += (position_cible[0] - joueur_pos[0]) * 0.1  # Ajustez le facteur pour la fluidité
    joueur_pos[1] += (position_cible[1] - joueur_pos[1]) * 0.1  # Ajustez le facteur pour la fluidité

    # Convertir les coordonnées du joueur en entiers pour l'accès à hopital
    joueur_pos_int = [int(joueur_pos[0]), int(joueur_pos[1])]
    ...
    pygame.draw.rect(fenetre, (255, 0, 0), (barre_endurance_x, barre_endurance_y, barre_endurance_longueur, barre_endurance_hauteur))  # Barre de fond
    pygame.draw.rect(fenetre, (0, 255, 0), (barre_endurance_x, barre_endurance_y, barre_endurance_longueur * (endurance_actuelle / endurance_max), barre_endurance_hauteur))  # Barre d'endurance

    # Collecte des clés
    if hopital[joueur_pos_int[1]][joueur_pos_int[0]] == "C":
        cles_collectees += 1
        hopital[joueur_pos_int[1]][joueur_pos_int[0]] = " "
    ...
    if hopital[joueur_pos_int[1]][joueur_pos_int[0]] == "S" and cles_collectees == NOMBRE_CLES:
        afficher_victoire()
        # Réinitialiser l'état du jeu si nécessaire
        afficher_menu()  # Retourner au menu principal
