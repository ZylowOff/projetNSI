...
def generer_hopital(nb_lignes, nb_colonnes):
    ...
    for _ in range(nb_lignes * nb_colonnes // 10):
        x, y = random.randint(
            1, nb_colonnes - 2), random.randint(1, nb_lignes - 2)
        if hopital[y][x] == " ":
    ...
    coins = [(0, 0), (0, nb_colonnes - 1), (nb_lignes - 1, 0),
        ...
        for j, case in enumerate(ligne):
            ...
            if distance_x <= 4 and distance_y <= 4:
                ...
                if 0 <= x < LARGEUR and 0 <= y < HAUTEUR:
                    if case == "#":
                        pygame.draw.rect(
                            fenetre, MUR, (x, y, TAILLE_CASE, TAILLE_CASE))
                    elif case == "S":
                        pygame.draw.rect(
                            fenetre, SORTIE, (x, y, TAILLE_CASE, TAILLE_CASE))
                    elif case == " ":
                        pygame.draw.rect(
                            fenetre, SOL, (x, y, TAILLE_CASE, TAILLE_CASE))

    # Dessiner le joueur
    joueur_ecran_x = joueur_x * TAILLE_CASE - camera_offset[0]
    joueur_ecran_y = joueur_y * TAILLE_CASE - camera_offset[1]
    pygame.draw.rect(fenetre, JOUEUR, (joueur_ecran_x,
                     joueur_ecran_y, TAILLE_CASE, TAILLE_CASE))
...
def menu_principal():
    bouton_jouer = pygame.Rect(
        (LARGEUR // 2 - 100, HAUTEUR // 2 - 50), (200, 100))
...
def menu_pause():
    bouton_continuer = pygame.Rect(
        (LARGEUR // 2 - 100, HAUTEUR // 2 - 150), (200, 75))
    bouton_recommencer = pygame.Rect(
        (LARGEUR // 2 - 100, HAUTEUR // 2 - 50), (200, 75))
    ...
    while True:
        ...
        texte_continuer = pygame.font.Font(
            None, 40).render("Continuer", True, BLANC)

        texte_recommencer = pygame.font.Font(None, 35).render(
            "Recommencer", True, BLANC)  # Taille réduite

        texte_quitter = pygame.font.Font(
            None, 40).render("Quitter", True, BLANC)

        fenetre.blit(texte_continuer, texte_continuer.get_rect(
            center=bouton_continuer.center))

        fenetre.blit(texte_recommencer, texte_recommencer.get_rect(
            center=bouton_recommencer.center))

        fenetre.blit(texte_quitter, texte_quitter.get_rect(
            center=bouton_quitter.center))
